...
const Inventario = () => {
  ...
  const datos = async () => {
    const info = await axios.get("http://localhost:5000/Inventario");
    setInventario(info.data);
    setFilteredInventario(info.data);
  };
  ...
  const handleDeleteClick = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/Inventario/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await axios.put(`http://localhost:5000/Inventario/${nuevoCarro.id}`, nuevoCarro);
      } else {
        await axios.post("http://localhost:5000/Inventario", nuevoCarro);
      }
      datos(); // Refresh the list of cars
      setNuevoCarro({
        Marca: "",
  ...
  return (
    <Box m="20px">
      <Header titulo="Inventario" subtitulo="Pagina de Inventario" />
      ...
      >
        <DataGrid rows={filteredInventario} columns={columnas} />
      </Box>
